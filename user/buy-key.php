...
<body class="min-h-screen flex flex-col" x-data="{ showLogoutConfirm: false, navOpen: false }">
    <nav class="p-4 glass border-b border-white/5 flex justify-between items-center px-6 md:px-12 sticky top-0 z-50">
        ...
        <div class="flex items-center gap-4">
            ...
            <div x-show="navOpen" @click.away="navOpen = false" x-cloak class="absolute right-6 top-20 w-64 bg-slate-900/95 backdrop-blur-xl rounded-[1.5rem] border border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)] py-3 overflow-hidden z-[60]">
                ...
                <a href="javascript:void(0)" @click="showLogoutConfirm = true; navOpen = false" class="flex items-center gap-3 px-4 py-3 hover:bg-red-500/10 text-red-400 text-sm font-bold transition-all border-t border-white/5"><?php echo getIcon('logout', 'w-5 h-5'); ?> Đăng xuất</a>
            </div>
        </div>
    </nav>

    <main class="p-6 max-w-7xl mx-auto w-full mt-8 px-6 md:px-12" x-data="{ selectedId: 1, quantity: 1, packages: <?php echo htmlspecialchars(json_encode($packages)); ?> }">
        ...
        <?php if ($success): ?>
            <div class="glass p-8 rounded-3xl border border-white/5 mb-8 text-center relative overflow-hidden animate-fade-in">
                ...
                <div class="warning-box p-3 rounded-2xl mb-6 relative z-10 max-w-lg mx-auto">
                    <p class="text-xs font-black text-red-400 flex items-center justify-center gap-2 uppercase tracking-widest">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
                        Lưu ý
                    </p>
                    <p class="text-xs text-slate-200 mt-1 font-semibold leading-relaxed whitespace-nowrap">Key chỉ dùng cho <span class="text-red-400 font-bold">1 thiết bị</span> & <span class="text-red-400 font-bold">không thể thay đổi</span> sau khi kích hoạt.</p>
                </div>
